...
def square_root(entry):
    """
    Acceptance Criteria:
    - Calculates the square root of the current value in the entry.
    - Clears the entry and displays the result.
...
def percentage(entry):
    """
    Acceptance Criteria:
    - Calculates the percentage of the current value in the entry.
    - Clears the entry and displays the result as a decimal (e.g., 0.5 for 50%).
    """
    pass
...
def popupmsg():
    ...
    B1.pack()
    B2 = Button(popup, text="Cancel", bg="#DDDDDD", command=popup.destroy)  
    B2.pack()
